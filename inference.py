...
for timestamp in [e for e in listdir('output') if 'lPhy' in e and '04-26-23' in e]:
    # timestamp = '03-21-23_12-38' ### insert timestamp here for which you want to predict output
    # timestamp = sys.argv[1]
    print("### exp " + str(timestamp) + " ###")
    out_root = '../../Outputs/Snow Radar/Thickness Regression - Hybrid'  ### output folder where you want to generate model predictions
    model_ext = '.h5'
    img_ext = '.png'
    txt_ext = '.txt'
    train = 'train'
    ...
    def predict_train(model, model_name):
        ...
        for file in files:
            ...
            np.savetxt(join(out_folder,file.replace(img_ext,txt_ext)),pred[0],fmt="%.3f",delimiter="\n") ### taking pred[0] because network has two copies of final FC layer
    ...
    for model_name in models:
        # model = load_model(join(models_in,model_name), custom_objects={'loss1': loss1, 'loss2': loss2})
